...
legend_entries = ["alpha = 100, beta = gamma = 0"];
...
cleaned_data_directory = sprintf('%s/cleaned_data', dir_names(1));
...
% Analytic solution to s
ode_fun = @(s) alpha * s - (8 * sqrt(3) / 5) * (ts - s).^(5/2);
s_analytic = fsolve(ode_fun, zeros(size(ts)));
plot(ts, s_analytic);
